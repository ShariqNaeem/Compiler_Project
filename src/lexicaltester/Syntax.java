...
//       }
    ...
    boolean F(){
        if(tokens[i].getClassPart().equals("!")){
            ...
            if(tokens[i].getClassPart().equals("super") && tokens[i+1].getClassPart().equals(".") && tokens[i+2].getClassPart().equals("ID")){
                i+=3;
                if(X()){
                    return true;
                }
            ...
            if(tokens[i].getClassPart().equals("this") && tokens[i+1].getClassPart().equals(".") && tokens[i+2].getClassPart().equals("ID")){
                i+=3;
                if(X()){
                    return true;
                }
...
//        
            ...
            if(Access_Modifier()){
                if(spec_kw()){
                    if(tokens[i].getClassPart().equals("class") && tokens[i+1].getClassPart().equals("ID")){
                        sv.setType("class");
                        sv.setName(tokens[i+1].getValuePart());
                        for (int j = 0; j <sym.clas.size(); j++) {
                            
                            if(sym.clas.get(j).name.equals(sv.name)){
                                System.out.println("Re-Declaration");
                                return false;
                            }
                        }
                        i+=2;
                        if(inherit()){
                            if(tokens[i].getClassPart().equals("{")){
                                ...
                                if(Class_Body()){
                                    if(tokens[i].getClassPart().equals("}")){
                                        i++;
                                        sym.insertClas(sv.name, sv.type, sv.AccModifier, sv.staticc, sv.category, sv.extend, sv.implement);
                                        sv.deleteScope();
                                        if(Class_Dec())
                                            return true;
                                        return true;
    ...
    boolean spec_kw(){
        if(SPEC_KW.contains(tokens[i].getClassPart())){
            if(tokens[i].getClassPart().equals("abstract") || tokens[i].getClassPart().equals("final")){
                sv.setCategory(tokens[i].getValuePart());
                i++;
                return true;
            }
            sv.setCategory("General");
            return true;
        }
        return false;
    }
    ...
    boolean inherit(){
        if(INHERIT.contains(tokens[i].getClassPart())){
            if(tokens[i].getClassPart().equals("extends") && tokens[i+1].getClassPart().equals("ID")){
                sv.setExtend(tokens[i+1].getValuePart());
                i+=2;
                return true;
            }
            sv.setExtend("-");
    ...
    boolean Access_Modifier(){
        if(ACCESS_MODIFIER.contains(tokens[i].getClassPart())){
            if(tokens[i].getClassPart().equals("AM")){
                sv.AccModifier=tokens[i].getValuePart();
                i++;
                return true;
            }
            sv.setAccModifier("private");
            return true;
        }
        return false;
    }
